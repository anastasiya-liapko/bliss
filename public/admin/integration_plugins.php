...
$next_order['url']='asc';
		...
		<script>
				...
				{
					$(\'.big-icon\').html(\'<i class="fas fa-plug"></i>\');
				};
				...
				<form method="POST">
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							<small>Вставьте сюда новые записи. Каждая запись на новой строчке: <b class="csv-create-format">Название, Логотип, Ссылка</b></small>
...
".(function_exists("processTD")?processTD("
	<div class='genesis-item-property '>
		...
		<span class='editable' data-placeholder='' data-inp='text' data-url='engine/ajax.php?action=editable&table=integration_plugins' data-pk='{$item['id']}' data-name='name'>".htmlspecialchars($item['name'])."</span>
	</div>", $item, "Название"):"
	<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=name&sort_order='. ($next_order['name']) .'\' class=\'sort\' column=\'name\' sort_order=\''.$sort_order['name'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['name'] ?? '<span class="fa fa-sort"></span>')).'</a>'."
...
".(function_exists("processTD")?processTD("
	<div class='genesis-item-property '>
		...
		<span class='editable' data-placeholder='' data-inp='text' data-url='engine/ajax.php?action=editable&table=integration_plugins' data-pk='{$item['id']}' data-name='url'>".htmlspecialchars($item['url'])."</span>
	</div>", $item, "Ссылка"):"
	<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=url&sort_order='. ($next_order['url']) .'\' class=\'sort\' column=\'url\' sort_order=\''.$sort_order['url'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['url'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
	...
	$actions['csv_create_execute'] = function()
	{
		...
		$sql = "INSERT IGNORE INTO integration_plugins (`name`, `img_url`, `url`) VALUES (?, ?, ?)";
...
$set[] = is_null($_REQUEST['url'])?"`url`=NULL":"`url`='".addslashes($_REQUEST['url'])."'";
	...
	function get_agregate()
	{
		...
		$sql = "SELECT 1 as stub  FROM (SELECT main_table.*  FROM integration_plugins main_table) temp $srch $filter $where $order";
