...
function processTD($html, $item, $column)
{
 return $html;
	...
	$actions[''] = function()
	{
   		...
   		$mfi_id_values = json_encode(q("SELECT CONCAT(name, ' (ID', id, ')') AS text, id as value FROM mfi", []));
				$mfi_id_values_text = "";
					foreach(json_decode($mfi_id_values, true) as $opt)
					{
					  $mfi_id_values_text.="<option value=\"{$opt['value']}\">{$opt['text']}</option>";
					}
$status_values = '[{"text":"Заявка одобрена", "value":"approved"},{"text":"Заявка отклонена", "value":"declined"},{"text":"МФО не успела дать ответ", "value":"did_not_have_time"}]';
		$status_values_text = "";
...
$next_order['id']='asc';
$next_order['request_id']='asc';
$next_order['mfi_id']='asc';
$next_order['status']='asc';
$next_order['time_response']='asc';
		...
		<script>
				...
				{
					$(\'.big-icon\').html(\'<i class="fas fa-receipt"></i>\');
				};
		...
		<div class="content-header">
			<div class="btn-wrap">
				<h2><a href="#" class="back-btn"><span class="fa fa-arrow-circle-left"></span></a> '."Ответы на заявки".' </h2>
			...
			<div class="form-group">
				<label style="display:block;">ID Заявки</label>
				<input type="number" min="-2147483648" max="2147483648" step="0.01" class="form-control filter-number-from" name="request_id_filter_from" value="'.$_REQUEST['request_id_filter_from'].'" placeholder="От" style="display:inline; width:49%;"/>
				<input type="number" min="-2147483648" max="2147483648" step="0.01" class="form-control filter-number-to" name="request_id_filter_to" value="'.$_REQUEST['request_id_filter_to'].'" placeholder="До" style="display:inline; width:49%; float:right;"/>
			</div>
						</div>
		...
		$table='
			...
			<div class="genesis-header-property">
				<nobr>
					<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=time_response&sort_order='. ($next_order['time_response']) .'\' class=\'sort\' column=\'time_response\' sort_order=\''.$sort_order['time_response'].'\'>Дата'. $sort_icon['time_response'].'</a>
					
			<span class=\'fa fa-filter filter btn btn-default\' data-placement=\'bottom\' data-content=\'<div class="input-group">
							<input autocomplete="off" type="text" class="form-control daterange filter-date-range" name="time_response_filter">
							<span class="input-group-btn">
								<button class="btn btn-primary add-filter" type="button"><span class="fa fa-filter"></a></button>
							</span>
						</div>\'>
...
".(function_exists("processTD")?processTD("<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=time_response&sort_order='. ($next_order['time_response']) .'\' class=\'sort\' column=\'time_response\' sort_order=\''.$sort_order['time_response'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['time_response'] ?? '<span class="fa fa-sort"></span>')).'</a>'."
			<span class='fa fa-filter filter ' data-placement='bottom' data-content='<div class=\"input-group\">
							<input autocomplete=\"off\" type=\"text\" class=\"form-control daterange filter-date-range\" name=\"time_response_filter\">
							<span class=\"input-group-btn\">
								<button class=\"btn btn-primary add-filter\" type=\"button\"><span class=\"fa fa-filter\"></a></button>
		...
		</span><span>".(new DateTime(($item['time_response']?$item['time_response']:"1970-01-01 00:00:00") ))->format('Y-m-d H:i')."</span></div>", $item, "Дата"):"<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=time_response&sort_order='. ($next_order['time_response']) .'\' class=\'sort\' column=\'time_response\' sort_order=\''.$sort_order['time_response'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['time_response'] ?? '<span class="fa fa-sort"></span>')).'</a>'."
			<span class='fa fa-filter filter ' data-placement='bottom' data-content='<div class=\"input-group\">
							<input autocomplete=\"off\" type=\"text\" class=\"form-control daterange filter-date-range\" name=\"time_response_filter\">
			...
			<span class='genesis-attached-column-name'>Дата:</span>
		</span><span>".(new DateTime(($item['time_response']?$item['time_response']:"1970-01-01 00:00:00") ))->format('Y-m-d H:i')."</span></div>")."
	...
	$actions['edit'] = function()
	{
		...
		if(isset($id))
		{
			$item = q("SELECT * FROM mfi_responses WHERE id=?",[$id]);
			$item = $item[0];
		}
	...
	$actions['edit_page'] = function()
	{
		...
		$html = '
			<h1 style="line-height: 30px"> Редактирование <br /><small>'."Ответы на заявки".' #'.$id.'</small></h1>
			<form class="form" enctype="multipart/form-data" method="POST">
	...
	$actions['reorder'] = function()
	{
		...
		{
			qi("UPDATE `mfi_responses` SET `` = ? WHERE id = ?", [$i, $line[$i]]);
		}
	...
	$actions['csv_create_execute'] = function()
	{
		...
		$sql = "INSERT IGNORE INTO mfi_responses () VALUES ()";
	...
	$actions['create_execute'] = function()
	{
		...
		$params = [];
		$sql = "INSERT INTO mfi_responses () VALUES ()";
	...
	$actions['edit_execute'] = function()
	{
		...
		if(!$skip)
		{
			...
			{
				$set = implode(", ", $set);
				$sql = "UPDATE mfi_responses SET $set WHERE id=?";
	...
	$actions['delete'] = function()
	{
		...
		try
		{
			qi("DELETE FROM mfi_responses WHERE id=?", [$id]);
	...
	function filter_query($srch)
	{
		...
		if(isset2($_REQUEST['request_id_filter_from']) && isset2($_REQUEST['request_id_filter_to']))
		{
			$filters[] = "request_id >= {$_REQUEST['request_id_filter_from']} AND request_id <= {$_REQUEST['request_id_filter_to']}";
		}
		...
		if(isset2($_REQUEST['time_response_filter_from']) && isset2($_REQUEST['time_response_filter_to']))
		{
			$filters[] = "time_response >= '{$_REQUEST['time_response_filter_from']}' AND time_response <= '{$_REQUEST['time_response_filter_to']}'";
		}
...
$status_values = '[{"text":"Заявка одобрена", "value":"approved"},{"text":"Заявка отклонена", "value":"declined"},{"text":"МФО не успела дать ответ", "value":"did_not_have_time"}]';
		...
		if(isset2($_REQUEST['request_id_filter_from']) && isset2($_REQUEST['request_id_filter_to']))
		{
			$filter_divs .= "
			<div class='filter-tag'>
					<input type='hidden' class='filter' name='request_id_filter_from' value='{$_REQUEST['request_id_filter_from']}'>
					<input type='hidden' class='filter' name='request_id_filter_to' value='{$_REQUEST['request_id_filter_to']}'>
					<span class='fa fa-times remove-tag'></span> ID Заявки: <b>{$_REQUEST['request_id_filter_from']}–{$_REQUEST['request_id_filter_to']}</b>
			</div>";
		...
		if(isset2($_REQUEST['time_response_filter_from']))
		{
			$from = date('d.m.Y', strtotime($_REQUEST['time_response_filter_from']));
			$to = date('d.m.Y', strtotime($_REQUEST['time_response_filter_to']));
			$filter_divs .= "
			<div class='filter-tag'>
					<input type='hidden' class='filter' name='time_response_filter_from' value='{$_REQUEST['time_response_filter_from']}'>
					<input type='hidden' class='filter' name='time_response_filter_to' value='{$_REQUEST['time_response_filter_to']}'>
	...
	function get_agregate()
	{
		...
		$sql = "SELECT 1 as stub  FROM (SELECT main_table.* , (select text FROM (SELECT CONCAT(name, ' (ID', id, ')') AS text, id as value FROM mfi) tmp_12514afa WHERE value=main_table.mfi_id) as mfi_id_text FROM mfi_responses main_table) temp $srch $filter $where $order";
	...
	function get_data($force_kill_pagination=false)
	{
		...
		if($pagination == 1)
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  main_table.* , (select text FROM (SELECT CONCAT(name, ' (ID', id, ')') AS text, id as value FROM mfi) tmp_12514afa WHERE value=main_table.mfi_id) as mfi_id_text FROM mfi_responses main_table) temp $srch $filter $where $order LIMIT :start, :limit";
		...
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT main_table.* , (select text FROM (SELECT CONCAT(name, ' (ID', id, ')') AS text, id as value FROM mfi) tmp_12514afa WHERE value=main_table.mfi_id) as mfi_id_text FROM mfi_responses main_table) temp $srch $filter $where $order";
			if(in_array($_SERVER['SERVER_NAME'], ["test-genesis.alef.im", "devtest-genesis.alef.im", "localhost"]) || $debug)
			{
				echo "<!--SQL DATA {$sql} -->";
			}
	...
	echo masterRender("Ответы на заявки", $content, 15);
