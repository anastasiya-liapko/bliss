...
import IssuedOrder from './IssuedOrder.jsx';
import IssuedOrdersTable from './IssuedOrdersTable.jsx';
...
class IssuedOrders extends Component {
  ...
  getFetchItemsUrl() {
    ...
    if (currentTabName === 'firstTab') {
      method = 'get-unpaid-orders';
    } else if (currentTabName === 'secondTab') {
      method = 'get-paid-orders';
    }

    return `${Helper.getLocation()}/shop-admin-panel/issued-orders/${method}`
  ...
  fetchItem(orderId) {
    const { currentTabName } = this.state;
    const url = `${Helper.getLocation()}/shop-admin-panel/issued-orders/get-order?id=${orderId}`;
  ...
  render() {
    ...
    if (firstTabShowItem) {
      firstTabInner = <IssuedOrder
        error={error}
        isLoading={isLoading}
        item={firstTabItem}
        onClose={this.handleCloseItem}/>;
    } else {
      firstTabInner = <IssuedOrdersTable
        hasEditButton={true}
        error={error}
        isLoading={isLoading}
        items={firstTabItems}
    ...
    if (secondTabShowItem) {
      secondTabInner = <IssuedOrder
        error={error}
        isLoading={isLoading}
        item={secondTabItem}
        onClose={this.handleCloseItem}/>;
    } else {
      secondTabInner = <IssuedOrdersTable
        error={error}
        isLoading={isLoading}
        ...
        <nav className="nav nav-tabs" role="tablist">
          <a onClick={this.handleChangeTab} className="nav-item nav-link active" id="firstTab" data-toggle="tab"
            ...
            <OverlayTrigger
              ...
              overlay={
                <Popover>
                  Отображаются заказы, доставленные покупателю и ожидающие перечисления оплаты от
                  финансовой организации, предоставившей кредит. Если оплата по заказу не поступила
                  в установленный срок, позвоните в финансовую организацию, они помогут её получить.
                </Popover>
              }>
          ...
          <a onClick={this.handleChangeTab} className="nav-item nav-link" id="secondTab" data-toggle="tab"
            ...
            <OverlayTrigger
              ...
              overlay={
                <Popover>
                  Отображаются заказы, доставленные покупателю и оплаченные финансовой организацией.
                  Проверьте свой расчетный счет :)
                </Popover>
              }>
              <i className="far fa-question-circle pl-2 text-muted" aria-hidden="true"/>
            </OverlayTrigger>
